...
```{r}
# import data
inter_state <- read.csv('Inter-StateWarData_v4.0.csv')
intra_state <- read.csv('Intra-StateWarData_v4.1.csv')
extra_state <- read.csv('Extra-StateWarData_v4.0.csv')
non_state <- read.csv('Non-StateWarData_v4.0.csv')
```
...
We first merged all four datasets to answer what types of wars happened most frequently. Based on *Figure 1*, we found that among all wat types, there are most inter-state wars (Type 1), civil wars for central control (Type 4) and civil wars for local issues (Type 5), which are expected types that happened in history.  
...
ggplot(total, aes(as.factor(WarType))) + 
  ...
  coord_flip() +
  xlab("War Type") +
  ggtitle("Figure 1.1: Bar chart of War Types (1816-2007)") + 
